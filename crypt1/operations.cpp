...
namespace  Operations{
    ...
    static byte reverse_sub_bytes_table[] = {
    ...
    void SubBytes(AES_CODE* input){
        for(int i=0;i<ARR_SIZE;++i){
            for(int j=0;j<ARR_SIZE;++j){
                *input[i][j] = sub_bytes_table[*input[i][j]];
            }
        }
    }
    ...
    void ShiftRows(AES_CODE* input){
        ...
        for(int i=0;i<ARR_SIZE;++i){
            shift = i;
            for(int j=0;j<ARR_SIZE;++j){
                res[i][j] = *input[i][(j + shift)%ARR_SIZE];
            }
            for(int j=0;j<ARR_SIZE;++j){
                *input[i][j] = res[i][j];
            }
    ...
    void MixColumn(byte* column){
        ...
        for (c = 0; c < ARR_SIZE; c++) {
            a[c] = column[c];
            h = (unsigned char)((signed char)column[c] >> 7);
            b[c] = column[c] << 1;
            b[c] ^= 0x1B & h;
        }
    ...
    void SubWord(byte* input){
        for(int i=0;i<ARR_SIZE;++i){
            input[i] = sub_bytes_table[input[i]];
        }
    }

    void RotWord(byte* input){
        const byte tmp = input[0];
        for(int i=0;i<ARR_SIZE-1;++i)
            input[i]= input[i+1];
        input[ARR_SIZE-1] = tmp;
    }
